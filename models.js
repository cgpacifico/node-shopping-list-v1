...
const Recipes = {
  ...
  update: function (req) {
    ...
    const allowedFields = ["name", "ingredients"]
    const itemId = req.params.id
    const recipe = this.items[itemId]
    if (!(recipe)) {
      throw StorageException(
        `Can't update item \`${itemId}\` because doesn't exist.`)
    }
    console.log(`Updating recipe with id \`${itemId}\``);
